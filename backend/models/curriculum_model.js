...
const getCurricula = async () => {
    let curr = [];
    const query = `SELECT c.id, c.title, GROUP_CONCAT(u_name) AS authors, GROUP_CONCAT(u.picture_url) AS author_picture_urls, GROUP_CONCAT(u.picture_url) AS author_picture_urls, c.semester, h.h_name, s.s_name, c.last_update, f.url_word, f.url_pdf, c.created_at, c.content, c.pic_url
                   FROM curricula c 
                   JOIN user_curriculum u_c ON c.id = u_c.cid 
                   JOIN users u ON u_c.uid = u.id 
                   JOIN homes h ON c.home_id = h.id 
                   JOIN subjects t ON c.subject_id = s.id 
                   LEFT JOIN files f ON c.file_id = f.id
                   GROUP BY c.id, c.title, c.semester, c.last_update, h.h_name, s.s_name, f.url_word, f.url_pdf, c.created_at, c.content, c.pic_url`;
...
const getCurriculumByHome = async (home) => {
    let curr = []
    const query = `SELECT c.id, c.title, GROUP_CONCAT(u_name) AS authors, GROUP_CONCAT(u.picture_url) AS author_picture_urls, c.semester, h.h_name, s.s_name, c.last_update, f.url_word, f.url_pdf, c.created_at, c.pic_url
                   FROM curricula c 
                   JOIN user_curriculum u_c ON c.id = u_c.cid 
                   JOIN users u ON u_c.uid = u.id 
                   JOIN homes h ON c.home_id = h.id 
                   ...
                   LEFT JOIN files f ON c.file_id = f.id
                   WHERE h.h_name = '${home}'
                   GROUP BY c.id, c.title, c.semester, c.last_update, h.h_name, s.s_name, f.url_word, f.url_pdf, c.created_at, c.pic_url`
    const [result] = await pool.execute(query);
    try{
        if(result.length > 0){
            for(var i = 0; i < result.length; i++){
...
const getCurriculumBySubject = async (subject) => {
    let curr = []
    const query = `SELECT c.id, c.title, GROUP_CONCAT(u_name) AS authors, GROUP_CONCAT(u.picture_url) AS author_picture_urls, c.semester, h.h_name, s.s_name, c.last_update, f.url_word, f.url_pdf, c.created_at, c.pic_url
                   FROM curricula c 
                   JOIN user_curriculum u_c ON c.id = u_c.cid 
    ...
    try{
        ...
        if(curr.length > 0){
            ...
            return {
                "message": `No Currirula of subject '${subject}'`,
                "code": "001"
            }
        }
    }catch(err){
...
const getCurriculumByUserId = async (id) => {
    ...
    const query = `SELECT c.id, c.title, 
                  ...
                  (SELECT GROUP_CONCAT(picture_url) FROM users WHERE id IN (SELECT uid FROM user_curriculum WHERE cid = c.id)) AS author_picture_urls,
                   c.semester, h.h_name, s.s_name, c.last_update, f.url_word, f.url_pdf, c.created_at, c.content, c.pic_url
                   FROM curricula c 
                   JOIN homes h ON c.home_id = h.id 
                   JOIN subjects t ON c.subject_id = s.id 
                   LEFT JOIN files f ON c.file_id = f.id
...
const getCurriculumByKeyWord = async (kw) => {
    ...
    const query = `SELECT c.id, c.title, 
                  ...
                  (SELECT GROUP_CONCAT(picture_url) FROM users WHERE id IN (SELECT uid FROM user_curriculum WHERE cid = c.id)) AS author_picture_urls,
                   ...
                   LEFT JOIN files f ON c.file_id = f.id
                   WHERE CONCAT(
                    c.title, c.semester, h.h_name, s.s_name, c.content,
...
const getCurriculumByID = async (id) => {
    const query = `SELECT c.id, c.title, GROUP_CONCAT(u.u_name) AS authors, c.semester, h.h_name, s.s_name, c.last_update, f.url_word, f.url_pdf, c.created_at, c.content, c.pic_url
                   FROM curricula c 
                   JOIN user_curriculum u_c ON c.id = u_c.cid 
                   JOIN users u ON u_c.uid = u.id 
                   JOIN homes h ON c.home_id = h.id 
